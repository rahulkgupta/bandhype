...
var tyaxistext = tyaxispath.append('text')
...
function barchart() {
    ...
    d3.json("getcity?city=" + promoterlocation[0] + "&state=" + promoterlocation[1].replace(/\s+/g, '') + "&start=" + start, function(json) {
        console.log(json)
...
function gettime(d, self) {
  ...
  // Add the y-axis.
    ...
    tyaxistext.attr("transform", "rotate(-90)")
                .attr("y", 6)
                .attr("dy", ".71em")
                .style("text-anchor", "end")
                .text("tweets");
  ...
  // Add a small label for the symbol name.
    ...
    return tooltip.style("visibility", "visible")
        .text("Tweet Percentage: " + Number((d.pct*100).toFixed(2)) + "%")
